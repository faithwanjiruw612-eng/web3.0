...
class AttentionModel(nn.Module):
    ...
    def _get_parallel_step_context(self, embeddings, state, from_depot=False):
        ...
        if self.is_vrp:
            ...
            if num_steps == 1:  # We need to special case if we have only 1 step, may be the first or not
                if state.i.item() == 0:
                    ...
                    return embeddings.gather(
                        1,
                        torch.cat((state.prev_a, current_node), 1)[:, :, None].expand(batch_size, 2, embeddings.size(-1))
                    ).view(batch_size, 1, -1)
            ...
            print("embeddings_per_step={}".format(embeddings_per_step))
           # print("num_steps={}".format(num_steps))
            return torch.cat((
                ...
                torch.cat((
                    embeddings_per_step[:,state.i.item()-1:state.i.item(), :].expand(batch_size, num_steps - 1, embeddings.size(-1)),
                    embeddings_per_step
                ), 2)
            ), 1)
